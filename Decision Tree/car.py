...
df_train = pd.read_csv("datasets/car/train.csv", names=column_headers)
df_test = pd.read_csv("datasets/car/test.csv", names=column_headers)
...
class DecisionTree:
    ...
    def _build_tree(self, X, y, depth=0):
        ...
        return Node(feature=best_feature, value=best_value, left=left_child, right=right_child)
        ...
        for feature in range(X.shape[1]):
            ...
            for value in values:
                ...
                if self.criterion == 'information_gain':
                    ...
                    gain = self._majority_error(y, y[left_indices], y[right_indices])
                elif self.criterion == 'gini_index':
                    gain = self._gini_index(y) - (len(left_indices) / len(y) * self._gini_index(y[left_indices]) + 
                                                    len(right_indices) / len(y) * self._gini_index(y[right_indices]))
...
if __name__ == "__main__":
    depths = range(1, 7)  # Set the range of depths to evaluate
