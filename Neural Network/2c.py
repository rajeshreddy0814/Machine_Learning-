...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size, initialize_zero=True):
        np.random.seed(42)
        self.weights = {
...
for hidden_size in widths:
    print(f"\nTraining with hidden size: {hidden_size} and zero initialization")
    nn_zero = NeuralNetwork(X_train.shape[1], hidden_size, 1, initialize_zero=True)
    learning_curve_zero = nn_zero.train(X_train, y_train, hidden_size, gamma_0, d, epochs)

    train_error_zero = 1 - np.mean(nn_zero.predict(X_train) == y_train)
    test_error_zero = 1 - np.mean(nn_zero.predict(X_test) == y_test)

    print(f"Zero Initialization - Training Error: {train_error_zero:.4f} | Test Error: {test_error_zero:.4f}")